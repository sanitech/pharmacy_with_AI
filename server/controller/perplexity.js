...
const drugSuggestionApi = async (req, res) => {
  ...
  if (!apiKey) {
    return res.status(500).json({ success: false, message: "Missing PERPLEXITY_API_KEY in environment." });
  }
...
Format the response in a clean and readable list.`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Perplexity API HTTP Error:", response.status, errorData);
      throw new Error(`Perplexity API HTTP Error: ${response.status}`);
    }
    ...
    if (!aiText) {
      ...
      throw new Error("Perplexity API returned invalid format - no text found");
    }
