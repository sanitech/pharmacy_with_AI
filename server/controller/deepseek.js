...
const drugSuggestionApi = async (req, res) => {
  ...
  if (!apiKey) {
    return res.status(500).json({ success: false, message: "Missing DEEPSEEK_API_KEY in environment." });
  }
...
Format the response in a clean and readable list.`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("DeepSeek API HTTP Error:", response.status, errorData);
      throw new Error(`DeepSeek API HTTP Error: ${response.status}`);
    }
    ...
    if (!aiText) {
      ...
      throw new Error("DeepSeek API returned invalid format - no text found");
    }
