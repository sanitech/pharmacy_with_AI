                        ...
                        <?php foreach ($pending_orders as $order_item): ?>
                            <div class="card mb-3 order-card">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div>
                                            <h6 class="mb-1">Order #: <?php echo htmlspecialchars($order_item['order_number']); ?></h6>
                                            <p class="text-muted mb-1">Customer: <?php echo htmlspecialchars($order_item['customer_name']); ?></p>
                                            <small class="text-muted">Created: <?php echo date('M d, Y H:i', strtotime($order_item['created_at'])); ?></small>
                                        </div>
                                        <div class="text-end">
                                            <form method="POST" style="display:inline-block;">
                                                <input type="hidden" name="order_id" value="<?php echo $order_item['id']; ?>">
                                                <button type="submit" name="action" value="approve_order" class="btn btn-sm btn-success">Approve</button>
                                            </form>
                                            <form method="POST" style="display:inline-block;">
                                                <input type="hidden" name="order_id" value="<?php echo $order_item['id']; ?>">
                                                <button type="submit" name="action" value="reject_order" class="btn btn-sm btn-danger">Reject</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
