...
// Get approved orders for processing
$approved_orders = $order->getAllOrders('approved');
        ...
        <ul class="nav nav-tabs" id="orderTabs" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="approved-tab" data-bs-toggle="tab" data-bs-target="#approved" type="button" role="tab">
                    <i class="fas fa-check-circle me-2"></i>Approved Orders (<?php echo count($approved_orders); ?>)
                </button>
            </li>
            <li class="nav-item" role="presentation">
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body text-center">
                                    ...
                                    <p class="text-muted">No orders are ready for payment.</p>
                                </div>
                            </div>
                        </div>
                    <?php else: ?>
                        <?php foreach ($approved_orders as $order_item): ?>
                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <div>
                                                <button class="btn btn-sm btn-outline-primary me-2"
                                                    onclick="viewOrder(<?php echo $order_item['id']; ?>, 'approved')">
                                                    <i class="fas fa-eye me-1"></i>View
                                                </button>
                                                <?php if ($order_item['status'] === 'approved'): ?>
                                                ...
                                                <?php elseif ($order_item['status'] === 'rejected'): ?>
                                                    <span class="text-danger ms-2">Order was rejected</span>
                                                <?php else: ?>
                                                    <span class="text-muted ms-2">Not yet approved</span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </div>
...
<script>
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (order.status === 'approved') {
                        ...
                        footer += `<span class="text-danger ms-2">Order was rejected by pharmacist.</span>`;
                    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (orderId) {
            ...
            if (window.approved_orders && window.approved_orders.includes(parseInt(orderId))) {
                status = 'approved';
            ...
            // If not found, default to approved
...
<script>
    window.approved_orders = [<?php echo implode(',', array_map(function ($o) {
                                    return $o['id'];
                                }, $approved_orders)); ?>];
